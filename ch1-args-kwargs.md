...
대부분 \*args와 \*\*kwargs는 함수를 정의 할 때 사용됩니다. \*args와 \*\*kwargs는 매개변수들을 함수에 넣어줍니다. 여기서 이 매개변수는 사용자들이 얼마나 많은 인자들을 함수에 넣을지 모르는 상황에서 앞의 두 \*args와 \*\*kwargs를 사용할 수 있을 것입니다.  
\*args는 **키워드 되지않은 **변수의 길이 매개변수 리스트를 함수에 보낼 때 사용합니다. 좀 더 명확하게 예를 들어보겠습니다.
...
\*\*kwargs는 **키워드 되지않은 **변수의 길이 매개변수 리스트 함수에 전달합니다.\(역주:  가장 큰 차이는 keyword이냐 nonkeyword이냐입니다.\) \*\*kwargs는 함수의 지정된 이름의 매개변수를 처리할 때 사용해야합니다. 아래의 예시를 보겠습니다.
...
함수에서 어떻게 키워드된 전달인자 리스트를  다룰 수 있는지 보았습니다. 위 예시는 단지 \*\*kwargs의 기본적인 방법일 뿐이고, 이것이 얼마나 유용한지 알게될 것입니다. \*args, \*\*kwargs를 전달 인자들의 리스트나 사전\(dictionary\)을 함수에서 호출하기위한 방법을 알아보겠습니다.
...
이제 위의 간단한 함수에서 전달인자를 통과시키기 위해 \*args 혹은 \*\*kwargs를 사용할 수 있습니다. 아래와 같이 사용합니다:
